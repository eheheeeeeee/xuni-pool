...
function getTransactionUrl (id, stats) {
	if (stats && blockExplorers) {
		return blockExplorers[stats.config.coin].transactionExplorer.replace('{symbol}', stats.config.symbol.toLowerCase())
			.replace('{id}', id);
	}
}
...
function getBlockchainUrl (id, stats) {
	if (stats && blockExplorers) {
		return blockExplorers[stats.config.coin].blockchainExplorer.replace('{symbol}', stats.config.symbol.toLowerCase())
			.replace('{id}', id);
	}
}
...
var translate = function (data) {
	...
	$("[data-tplaceholder]")
		.each(function (index) {
			var strTr = data[$(this)
				.attr('data-tplaceholder')];
			$(this)
				.attr('placeholder', strTr)
		});
...
const $_GET = {};
const args = location.search.substr(1)
	.split(/&/);
for (var i = 0; i < args.length; ++i) {
	const tmp = args[i].split(/=/);
	if (tmp[0] != "") {
		$_GET[decodeURIComponent(tmp[0])] = decodeURIComponent(tmp.slice(1)
			.join("")
			.replace("+", " "));
		langCode = $_GET['lang'];
	}
...
function loadTranslations () {
	if (langData) {
		...
		$.getScript('../assets/lang/timeago/jquery.timeago.' + langCode + '.js');
	} else {
		$.getJSON('../assets/lang/' + defaultLang + '.json', translate);
		$.getScript('../assets/lang/timeago/jquery.timeago.' + defaultLang + '.js');
	}
}
...
function top10Miners_GetMinerCells (position, data) {
	...
	return '<td class="col1" data-sort="' + position + '">' + position + '</td>' +
		...
		'<td class="col4" data-sort="' + lastShare + '">' + (lastShare ? $.timeago(new Date(parseInt(lastShare) * 1000)
			.toISOString()) : 'Never') + '</td>' +
...
function settings_Setup (api, stats) {
	...
	$(`#payoutSetButton${stats.config.coin}`)
		.click(function () {
			var address = $(`#yourAddress${stats.config.coin}`)
				.val()
				.trim();
			if (!address || address == '') {
			...
			var ip = $(`#yourIP${stats.config.coin}`)
				.val()
				.trim();
			if (!ip || ip == '') {
			...
			var level = $(`#yourPayoutRate${stats.config.coin}`)
				.val()
				.trim();
			if (!level || level < 0) {
	...
	$(`#enableButton${stats.config.coin}`)
		.click(function () {
			var address = $(`#yourAddress${stats.config.coin}`)
				.val()
				.trim();
...
function settings_ShowError (id, message, extra, stats) {
	...
	$(`#action_update_message${stats.config.coin}`)
		.text(message);
	$(`#action_update_message${stats.config.coin}`)
		.removeClass()
	...
	$.ajax({
		...
		.done(function (data) {
			if (data.level != undefined) {
				$(`#yourPayoutRate${stats.config.coin}`)
					.val(data.level);
			}
		});
}
...
function settings_InitTemplate (ranOnce) {
	if (!lastStats.config.sendEmails) $(`#emailNotifications${lastStats.config.coin}`)
		.hide();
		...
		.forEach(key => {
			if ($(`#blocksTabs li:contains(${key})`)
				...
				if (!mergedStats[key].config.sendEmails)
					$(`#emailNotifications${mergedStats[key].config.coin}`)
					.hide();
				template = $('#siblingTemplate')
					.html()
				Mustache.parse(template)
	...
	$(`#loadMorePayments${stats.config.coin}`)
		.click(function () {
			...
			xhrGetPayments[stats.config.coin] = $.ajax({
				...
				data: {
					time: $(`#paymentsReport${stats.config.coin}_rows`)
						.children()
						.last()
						.data(`time`)
				},
...
function market_UpdateMarkets (api, stats, currencyPairs, xhrMarketGets, marketPrices) {
	...
	for (let i = 0; i < marketCurrencies.length; i++) {
		currencyPairs[stats.config.coin].push(marketCurrencies[i].replace('{symbol}', stats.config.symbol)
			.toUpperCase());
	}
		...
		for (var graphType in graphData) {
			if (graphData[graphType].data[0].values.length > 1) {
				...
				if (graphData[graphType].data[1]) {
					...
					settings.tooltipFormat = '<span style="color:#ddd">SOLO: {{y}}</span> &ndash; {{offset:names}}'
