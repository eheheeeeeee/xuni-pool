...
function loadTranslations() {
    if (langData) {
        ...
        $.getJSON('../dist/lang/' + langCode + '.json', translate);
        $.getScript('../dist/lang/timeago/jquery.timeago.' + langCode + '.js');
    } else {
        $.getJSON('../dist/lang/' + defaultLang + '.json', translate);
        $.getScript('../dist/lang/timeago/jquery.timeago.' + defaultLang + '.js');
    }
}
...
function renderLangSelector() {
    ...
    var numLangs = 0;
    if (langs) {
        html += '<select id="newLang" class="selectize-control p-1 rounded">';
